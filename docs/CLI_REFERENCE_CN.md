...
flow 模式 turbo（可选，默认 `no`；仅在 `--mux-mode=flow` 下有意义）。新连接的首包通过当前
质量最好的承载链路（依据心跳信号）发送，并在后台预热额外承载链路以扩大竞争池——在不绑定连接
的前提下改善首包时延。可在 JSON 中设置 `mux.turbo`，或传 `--mux-mode-turbo=yes`。
