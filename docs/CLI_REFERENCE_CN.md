...
MUX 调度模式，决定排队帧如何分散到底层 mux linklayer。它**只是发送端策略**：VMUX 线
协议仍使用单一全局 seq/ack 窗口，接收端始终按全局顺序交付。所有模式都不改变线格式，
且该选项不协商——如需双向行为一致，请两端配置相同。

| 模式 | 行为 | 适用场景 |
|------|------|----------|
| `compat` | 现有调度器；排队帧发往任意空闲 linklayer。 | 默认 / 回退 / 回归基线。 |
| `flow` | 单流粘滞：选一条活跃 linklayer 作 primary，排队帧只走它，保留单流顺序/吞吐。 | 抖动链路上的单流性能。 |
| `balance` | 按连接粘滞负载均衡：每个逻辑连接绑定到负载最低的活跃 linklayer（首次轮询选择，链路掉线则迁移）；连接内的帧在该链路有发送额度时保持粘滞。 | 多并发连接、希望吞吐分散到多链路。 |
| `stripe` | 实验性切片：排队帧不分连接，轮询分发到所有活跃 linklayer。 | 未来伪 MPTCP / 9000 MTU 工作的基础。 |

> **`balance` 注意：** 因为接收端仍用单一全局 seq/ack 窗口，负载均衡能提升多连接场景的
> 链路利用率，但**不能消除接收端 HOL**；慢链路仍可能卡住全局交付。真正的 per-flow 交付
> 需要接收端协议改造（见 issue #5 设计说明）。
>
> **`stripe` 注意（实验性）：** 把单条逻辑流分散到速度不同的链路会导致接收端大量乱序
> （缓存在乱序队列里），可能使单流性能**劣于** `compat`/`flow`。它是为后续 per-link 序号 /
> DSN 工作预留的实验基座，当前不是快速路径。
