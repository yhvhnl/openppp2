...
MUX 调度模式，决定排队帧如何分散到底层 mux linklayer。`compat` 是单一全局 VMUX seq/ack
窗口下的纯发送端策略。`flow`、`balance`、`stripe` 会把一个会话的帧分散到**多条链路**，
因此会**自动协商接收端按流定序（flow v2）**，以免跨链路乱序被误判为丢包。该协商是交集：
仅当**两端**都运行支持的版本时才启用；否则会话透明回退到单链路的 `compat` 定序行为。所有
模式都不改变线格式。如需双向行为一致，请两端配置相同。

| 模式 | 行为 | 适用场景 |
|------|------|----------|
| `compat` | 现有调度器；排队帧发往任意空闲 linklayer。 | 默认 / 回退 / 回归基线。 |
| `flow` | 协商到 flow v2 时按连接粘滞分散到多链路：每个连接固定绑定一条链路（保留其 per-flow 顺序），不同连接走不同链路，使一个连接的大流量无法对另一连接的首包造成队头阻塞。对端不支持 flow v2 时回退为单一 primary 链路。 | 多链路下的单流时延 / 首包响应速度。 |
| `balance` | 按连接粘滞负载均衡：每个逻辑连接绑定到负载最低的活跃 linklayer（首次轮询选择，链路掉线则迁移）；连接内的帧在该链路有发送额度时保持粘滞。自动协商 flow v2。 | 多并发连接、希望吞吐分散到多链路。 |
| `stripe` | 实验性切片：排队帧不分连接，轮询分发到所有活跃 linklayer。自动协商 flow v2。 | 未来伪 MPTCP / 9000 MTU 工作的基础。 |

> **flow v2 需要两端均为支持的版本。** `flow`/`balance`/`stripe` 只有在协商成功时才获得
> 多链路收益；对端为旧版本时回退到单链路交付（不会回归，但也不会有提升）。也可在 JSON 中
> 用 `mux.flow-v2: true` 显式强制广告该能力。
>
> **`stripe` 注意（实验性）：** 把单条逻辑流分散到速度不同的链路会导致接收端大量乱序
> （缓存在 per-flow 乱序队列里），可能使单流性能**劣于** `compat`/`flow`。它是为后续 per-link
> 序号 / DSN 工作预留的实验基座，当前不是快速路径。
