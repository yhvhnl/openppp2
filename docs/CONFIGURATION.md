...
- `mux.mode` — scheduler mode (`compat` | `flow` | `balance` | `stripe`). All modes send by competition (any free link takes the next frame; no per-connection binding). `compat` is the original upstream behavior (global ordering). `flow` is latency-oriented (global ordering; pairs with `mux.turbo`). `balance` adds negotiated per-flow receiver ordering (flow v2) so a connection only head-of-line blocks itself. `stripe` is experimental. An unrecognized value normalizes to `compat` with a non-fatal startup warning.
- `mux.turbo` — flow-mode turbo (default false): best-link-first first packet plus prewarmed carrier links to widen the competition pool. Only meaningful under `mux.mode=flow`. CLI: `--mux-mode-turbo=[yes|no]`.
